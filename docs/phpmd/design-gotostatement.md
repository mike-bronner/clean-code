...
the construct itself. Running `phpcs` with `CleanCode/ruleset.xml` therefore covers this
...
- **Severity raised to error** — the sniff reports a *warning* out of the box.
  `CleanCode/ruleset.xml` overrides it with `<type>error</type>` so that `goto` fails a
...
- **Message replaced** — the stock text is "Use of the GOTO language construct
  ...
  remedy. `CleanCode/ruleset.xml` overrides it with "Use of the goto construct is
  ...
  `CleanCode/ruleset.xml` refers to `Generic.PHP.DiscourageGoto.Found` rather than to the
  sniff name.
