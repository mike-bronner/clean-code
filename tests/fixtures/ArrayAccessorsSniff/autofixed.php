...
class ArrayAccessorsFailing
{
    ...
    {
        $name = data_get($payload, ['items', 0, 'name']);
    ...
    {
        $keyed = [data_get($payload, 'id') => data_get($payload, 'name')];
        $target[data_get($payload, 'index')] = 'set';
        $flags = $mask & data_get($payload, 'flags');

        foreach (data_get($payload, 'rows') as $row) {
            $keyed[] = $row;
        }

        $label = strtoupper(data_get($payload, 'label'));
    ...
    public function readsComputedIndexesInsideWriteTargets(
        ...
        foreach ($rows as $target[data_get($payload, 'value')]) {
        }

        foreach ($rows as $target[data_get($payload, 'key')] => $ignored) {
        }

        foreach ($rows as $order->{data_get($payload, 'member')}) {
        }

        foreach ($rows as $target[strtolower(data_get($payload, 'computed'))]) {
        }

        [$target[data_get($payload, 'first')]] = $source;
        list($target[data_get($payload, 'second')]) = $source;
        ['x' => $target[data_get($payload, 'third')]] = $source;
        [$target[strtolower(data_get($payload, 'fourth'))]] = $source;
    ...
    public function readsComputedIndexesSpanningBracesAndStatements(
        ...
        foreach ($rows as $target[match (true) { default => data_get($payload, 'matched') }]) {
        }

        foreach ($rows as $target[(function () use ($payload) { return data_get($payload, 'returned'); })()]) {
        }

        foreach ($rows as $target[(static function () use ($payload) { return data_get($payload, 'static'); })()]) {
        }

        [$target[match (true) { default => data_get($payload, 'first') }]] = $source;
        list($target[match (true) { default => data_get($payload, 'second') }]) = $source;
        [$target[strtolower(match (true) { default => data_get($payload, 'third') })]] = $source;
        list($target[(function () use ($payload) { return data_get($payload, 'fourth'); })()]) = $source;
    ...
    {
        if (isset($payload['present']) && data_get($payload, 'live') === 'x') {
            return true;
        }
    ...
    public function readsAcrossANestedForeachClause(
        ...
        foreach (mapRows(data_get($rows, 'pending'), function (array $row) use ($inner, $trailing): void {
            foreach ($row as $inner['each']) {
            }

            $ignored = data_get($trailing, 'after');
        }) as $outer['value']) {
        }
    }
    ...
    public function readsInAForeachTargetHoldingANestedForeachClause(
        ...
        foreach (data_get($trapRows, 'pending') as $trapTarget[(function () use ($trapInner, $trapOffset): string {
            foreach (data_get($trapOffset, 'each') as $trapInner['row']) {
            }
    ...
    public function readsComputedDynamicMembersSpanningBraces(
        ...
        foreach ($rows as $order->{match (true) { default => data_get($payload, 'member') }}) {
        }

        foreach ($rows as $order->{(function () use ($payload) { return data_get($payload, 'named'); })()}) {
        }
