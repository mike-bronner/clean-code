...
- [Classes: Class Naming](docs/standards/classes-class-naming.md) — Tier 2, custom sniff `CleanCode.Classes.ClassSuffixByFolder`: requires the class suffix implied by its folder under `app/` (`app/Services/PaymentService`, `app/Http/Controllers/UserController`), with `app/Models` and `app/Livewire` exempt and `app/Livewire/Forms` reserved for `Form`-suffixed classes, detection-only ([#27](https://github.com/mike-bronner/phpcs-rules/issues/27))
