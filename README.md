...
- [Dependency Injection](docs/standards/dependency-injection.md) — Tier 3, no rule: whether a collaborator *should* be injected is an architectural judgement, and the cross-file heuristic a sniff would need is out of reach for a single-file token scan, so the standard stays with code review ([#72](https://github.com/mike-bronner/phpcs-rules/issues/72)); the token-visible slice — `new` inside constructor bodies — has its own focused sniff issue ([#176](https://github.com/mike-bronner/phpcs-rules/issues/176))
