...
- [Pattern: Don't Repeat Yourself (DRY)](docs/standards/pattern-dont-repeat-yourself-dry.md) — Tier 2, custom sniff `CleanCode.Functions.AvoidDuplicateFunctionBodies`: warns once per function/method body in a file whose normalized token stream (comments and whitespace stripped, signature ignored) exactly matches an earlier one, above a configurable statement threshold, detection-only; whether a flagged duplication has earned an abstraction, and duplicated *knowledge* that is not duplicated text, stay with code review ([#134](https://github.com/mike-bronner/phpcs-rules/issues/134))
