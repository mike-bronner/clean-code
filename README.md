...
- [Testing: Guidelines](docs/standards/testing-guidelines.md) — Tier 3, no rule: the standard turns on process and cross-file judgements (where testing started, whether every scenario has success *and* failure coverage, whether a mocked interface is one the team controls) that a single file's token stream does not carry, so the semantic core stays with code review ([#54](https://github.com/mike-bronner/phpcs-rules/issues/54)); the two token-visible slices are tracked as focused sniff issues ([#145](https://github.com/mike-bronner/phpcs-rules/issues/145), [#146](https://github.com/mike-bronner/phpcs-rules/issues/146))
