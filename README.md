...
- [PHPMD/CleanCode: UndefinedVariable](docs/standards/phpmd-clean-code-undefined-variable.md) — Tier 1, external standard `VariableAnalysis.CodeAnalysis.VariableAnalysis` (sirbrillig/phpcs-variable-analysis): warns on reads of a variable the scope never assigns, including array elements, string interpolation, and `unset()` arguments; the sniff's unused-variable, redeclaration, and static-scope codes are excluded as other rules' concerns, detection-only. Stricter than PHPMD on two shapes, and neither tool catches a name assigned on one branch only — see the doc ([#85](https://github.com/mike-bronner/phpcs-rules/issues/85))
