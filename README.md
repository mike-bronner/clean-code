...
- [UnusedCode: UnusedFormalParameter](docs/phpmd/unusedcode-unusedformalparameter.md) — Tier 1, bundled sniff `Generic.CodeAnalysis.UnusedFunctionParameter` raised to error severity with the six inherited-signature codes excluded, report-only; stricter than PHPMD on closures, arrow functions, and `func_get_args()`, looser on three shapes ([#120](https://github.com/mike-bronner/phpcs-rules/issues/120))
