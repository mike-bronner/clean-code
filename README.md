...
- [Constructors: No Logic in Constructors](docs/standards/constructors-no-logic-in-constructors.md) — Tier 2, custom sniff `CleanCode.Constructors.NoLogic`: flags every non-assignment statement in a `__construct()` body, detection-only ([#40](https://github.com/mike-bronner/phpcs-rules/issues/40))
