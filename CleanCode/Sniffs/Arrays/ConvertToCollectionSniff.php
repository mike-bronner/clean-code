 ...
 * name (App\Support\array_map()) are all different symbols and stay out. A
...
class ConvertToCollectionSniff implements Sniff
{
     ...
     * replaces each one. Configurable from a ruleset via
     * <property name="arrayFunctions" type="array" value="array_map=>map"/>.
     *
     * PHPCS also accepts a plain comma-separated list there
     * (value="array_walk,array_values"), which its ruleset parser hands over
     * numerically keyed rather than as a name => method map. Those entries
     * still configure the sniff — replacements() derives the Collection method
     * for them by dropping the array_ prefix — so the natural list spelling
     * cannot silently switch the sniff off.
     *
    ...
    private function replacements(): array
    {
        ...
        foreach ($this->arrayFunctions as $key => $value) {
            ...
            $function = $isListEntry === true ? $value : (string) $key;
            $method = $isListEntry === true ? $this->collectionMethod($function) : $value;

            $replacements[strtolower($function)] = $method;
        }
     ...
     * array_reverse => reverse). Give the map spelling instead wherever the
     * two names diverge, e.g. value="usort=>sortBy".
     */
     ...
     * (App\Support\array_map, namespace\array_map) rather than a
     * fully-qualified global one (\array_map). Qualified names resolve outside
     * the global namespace, so they are never the native array functions.
     */
